...
单栏复习左侧中部提供「← 上一张卡片」（窄窗显示箭头），底部评分区独立居中；双栏保留各栏底部入口，撤销最近评分并返回重新作答，同步恢复原评分的调度与复习记录。支持连续返回、两档／四档、高级底栏和双栏；没有可撤销评分时置灰，双栏按实际评分顺序回到原栏。原有 `Ctrl+Z` 保持可用，见 [返回上一张说明](docs/REVIEW-UNDO.md)。
