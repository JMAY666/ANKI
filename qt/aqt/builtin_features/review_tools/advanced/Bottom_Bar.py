...
config = dict(get_config("advanced_review"))
for key, value in config.items():
    if key.startswith("Button Label_"):
        config[key] = escape(tr(value))
...
{middleLeftSide_button3}
{middleLeftSide_button4}
<button title="快捷键：空格" onclick='pycmd("ans");' {showAnswer_style}>{showAnswer_text}</button>
{middleRightSide_button1}
{middleRightSide_button2}
{middleRightSide_button3}
{middleRightSide_button4}
