...
from aqt.builtin_features import asset_url, theme_dir
...
def _launcher_sidebar_icon_url(is_open: bool) -> str:
    ...
    return asset_url(f"media/sidebar_icon_{state}.svg")
...
def inject_theme_assets(web_content: WebContent, context: Optional[Any]):
    ...
    try:
        ...
        local_path = str(theme_dir() / css_file)
        
        if not os.path.exists(local_path):
            local_path = str(theme_dir() / "medical_theme.css")
            css_file = "medical_theme.css"

        if os.path.exists(local_path):
            web_content.css.append(asset_url(f"themes/{css_file}?v={int(os.path.getmtime(local_path))}"))
            # Card templates/editors are untrusted documents. Their native CSP
            # forbids the dashboard's inline scripts; only style them here.
            if not isinstance(context, (DeckBrowser, Overview, TopToolbar,
                                        ReviewerBottomBar, DeckBrowserBottomBar,
                                        OverviewBottomBar)):
                return
