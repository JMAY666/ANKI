...
class BrowserWorkspace(QObject):
    ...
    def set_enabled(self, enabled, closing=False):
        ...
        if enabled:
            ...
            self.status.show()
            self._set_columns(["question", "template", "cardDue", "target_retrievability"])
            ...
            self._restore_header(self.standard["card_header"])
            if self.standard["notes"]:
                ...
                b._switch.blockSignals(False)
            self._restore_header(self.standard["header"])
    ...
    def _set_columns(self, columns, closing=False):
        ...
        header = self.browser.form.tableView.horizontalHeader()
        if not closing:
    ...
    def _restore_header(self, state):
        # Restoring a saved column layout must not simulate a user's sort click.
        # In particular, Target R changes section indices between both layouts.
        header = self.browser.form.tableView.horizontalHeader()
        blocked = header.blockSignals(True)
        try:
